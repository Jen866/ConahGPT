...
# Load credentials from environment variable for security
# In your Render setup, you will need to set an environment variable
# named SERVICE_ACCOUNT_JSON with the content of your creds file.
...
def list_all_files_in_folder(folder_id):
    """Lists all supported files in a given folder."""
    query = f"'{folder_id}' in parents and (mimeType='application/vnd.google-apps.document' or mimeType='application/vnd.google-apps.spreadsheet' or mimeType='application/pdf')"
...
def read_google_sheet_by_row(file_id):
    ...
    try:
        ...
        for record in records:
            # Only join non-empty values for cleaner text
    ...
    try:
        ...
        for content_item in doc_content:
            if 'paragraph' in content_item:
                ...
                # Find a bookmark ID if one exists for this paragraph
...
def extract_all_chunks_with_links(folder_id):
    """
    ...
    for file in files:
        ...
        if mime_type == 'application/vnd.google-apps.spreadsheet':
            ...
            for page_info in read_pdf_by_page(file_id):
                # Chunk the text from a single PDF page if it's very long
